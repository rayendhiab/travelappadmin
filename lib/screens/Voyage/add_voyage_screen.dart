...
class _AddVoyageScreenState extends State<AddVoyageScreen> {
  ...
  Future<bool> _getDataAsync(BuildContext context) async {
    if (widget.id.isNotEmpty) {
      final voyage = Provider.of<DataProvider>(context, listen: false)
          .voyages
          .firstWhere((element) => element.id == widget.id);
      //  await Future.delayed(const Duration(seconds: 1), () {
      _formData.id = widget.id;
      _formData.name = voyage.name;
      _formData.numMax = voyage.numMax;
      _formData.prix = voyage.prix;
      _formData.address = voyage.address;
      // });
    }
  ...
  void _doSubmit(BuildContext context) {
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      final dialog = AwesomeDialog(
        ...
        btnOkText: "Yes",
        btnOkOnPress: () async {
          await Provider.of<DataProvider>(context, listen: false).addVoyage(
              _formData.name,
              _formData.numMax,
              _formData.address,
              _formData.prix,
  ...
  Widget build(BuildContext context) {
    ...
    final pageTitle = 'Offer - ${widget.id.isEmpty ? "Ajouter" : "Detail"}';
