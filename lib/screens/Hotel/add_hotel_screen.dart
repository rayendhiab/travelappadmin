...
class _AddHotelScreenState extends State<AddHotelScreen> {
  ...
  Future<bool> _getDataAsync(BuildContext context) async {
    if (widget.id.isNotEmpty) {
      final hotel = Provider.of<DataProvider>(context, listen: false)
          .hotels
          .firstWhere((element) => element.id == widget.id);
      //  await Future.delayed(const Duration(seconds: 1), () {
      _formData.id = widget.id;
      _formData.name = hotel.name;
      _formData.stars = hotel.stars;
      _formData.prix = hotel.prix;
      _formData.address = hotel.address;
      // });
    }
  ...
  void _doSubmit(BuildContext context) {
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      final dialog = AwesomeDialog(
        ...
        btnOkText: "Yes",
        btnOkOnPress: () async {
          await Provider.of<DataProvider>(context, listen: false).addHotel(
              _formData.name,
              _formData.stars,
              _formData.address,
              _formData.prix,
  ...
  Widget build(BuildContext context) {
    ...
    final pageTitle = 'Employee - ${widget.id.isEmpty ? "Ajouter" : "Detail"}';
        ...
        children: [
          ...
          Padding(
            ...
            child: FormBuilderTextField(
              ...
              initialValue: _formData.stars.toString(),
              validator: FormBuilderValidators.required(),
              keyboardType:
                  const TextInputType.numberWithOptions(decimal: false),
              onSaved: (value) => (_formData.stars = int.parse(value!)),
            ),
          ),
          Padding(
